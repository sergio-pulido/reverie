...
| Cross-instance serialization of script mutations | Closed as a deployment constraint (RV-22) | `withJamLock` and the per-jam outline queue in `apps/server/outline.ts`; `JamStore.commitScript` guards on the boundary and the base revision | The jams router runs only in the container process the director already requires (`docs/DECISIONS.md` 2026-09-20), so the per-jam critical section is real there. Deploying these routers as Vercel functions is not a supported deployment for edits |
| Story outline projection (beats over the script) | Implemented | `src/core/outline.ts` (`buildOutline`, `beatAt`); `summary` on the portion schema; served by `GET /api/jams/:id/outline` (`apps/server/outline.ts`); rendered by `src/screens/OutlinePanel.tsx` on the script screen and in the Studio | Beats are created with the script (`apps/server/outlineWriter.ts`); a revision created before RV-22 shows its beats as missing |
| Outline cascade (re-derive the tail from an edited beat) | Implemented, unprobed | `src/core/outlineCascade.ts` (prompt for `set` and `reroll`, reply schema, `applyCascade`); `runCascade` in `apps/server/outlineWriter.ts` calls Nebius with one retry | No cascade has been run against a live provider from this repository; exercised with injected completions only |
| Outline edit queue (serialized, envelope-guarded) | Implemented (local only) | `OutlineEditQueue` and the routes in `apps/server/outline.ts`; `outlineEditCommandSchema` in `src/core/outlineEdit.ts` (`requestId`, `expectedRevision`, `mechanism`, `authorId`); `JamStore.commitScript` | In-memory ledger, lost on restart with the script; no authorization on the routes; `jam_ended` refusal waits on RV-18's lifecycle field |
| Ways to modify the outline (direct, vote, poll, chat) | Direct implemented; vote, poll, chat not implemented | The panel's "Rewrite" (`set`) and "Not this" (`reroll`) in `src/screens/OutlinePanel.tsx`, `mechanism: "direct"` | No tally, poll or chat adapter; `jam_proposals` remains append-only with no update policy |
| Beat delivered as live director direction | Implemented, unprobed | After a commit the queue calls `stream.direct({ body, beatIndex, authorId })` on every stream `DirectorStreamRegistry.streamsFor(jamId)` returns; refusals land on the edit record | Never exercised against a live stream; the fake peer in tests records the prompt |
