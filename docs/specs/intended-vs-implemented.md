...
| Story outline projection (beats over the script) | Implemented | `src/core/outline.ts` (`buildOutline`, `beatAt`) over `src/core/directorBeats.ts`'s timeline; `summary` on the portion schema, `src/core/script.ts` | No route serves it; no client surface renders it |
...
| Beat delivered as live director direction | Not implemented | Consumer exists on main (`apps/server/directorStream.ts`); `DirectionRequest` carries `beatIndex?`; `beatAt` computes the current beat | Nothing calls `direct` with a beat; no producer is wired |
