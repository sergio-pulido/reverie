...
- `POST /session/:sessionId/end` — `{ viewerId? }` → `204`. With a viewer id, that viewer leaves and the session ends only when nobody is left watching **by any route**; without one the session ends outright, which is the host's stop. Two viewer notions coexist — relay peers and counted viewer ids — and a session survives while either has an audience.
...
Segments are produced whenever any sink wants them: the live window when HLS delivery is on, and any sink registered through the router's `createSegmentSinks` seam — which is where a durable archive attaches — with the same numbering, so a stored session and a live one describe one timeline.
