 ...
 */
export function CreateRoom({ title, premise, visibility, sourceKind, importedScript, totalSeconds, portionMinSeconds, portionMaxSeconds, onTitle, onPremise, onVisibility, onSourceKind, onImportedScript, onTotalSeconds, onPortionMinSeconds, onPortionMaxSeconds, onSubmit, isCreating, notice }: CreateRoomProps) {
  return <main className="site-shell setup-shell"><TopBar current="jam" /><section className="setup-layout setup-layout-explained">
    ...
    <form className="room-form" onSubmit={onSubmit}>
      ...
      <button className="button button-primary form-submit" type="submit" disabled={isCreating}>{isCreating ? `Writing your ${totalSeconds}-second script…` : "Write the script"}<span>↗</span></button>
