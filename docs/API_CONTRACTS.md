...
`GET /api/jams/:id/director/budget` answers the same `spend` with `sessionUsd: 0`, plus `configured` — whether a director is configured on this server at all, which is a different fact from having money left — `maxSessionSeconds`, and `filmSeconds`, the runtime of the jam's script. The budget belongs to the process, not to a room, so a jam this server does not hold still gets an answer: `filmSeconds` is `null`, which means unknown, never endless.
...
A take also ends when a viewer has **watched** to the end of the film, which on any script shorter than the ceiling is what it reaches first. The trigger is the media element's own position against the script's runtime (`playedToEnd`, `src/core/directorCompletion.ts`), sent by the browser as the ordinary whole-room stop with `reason: "played_to_end"` — the same teardown a pressed Stop runs, so the session settles, the archive closes and the room leaves `playing` on one path. It is deliberately NOT keyed on the provider's progress: generation finishes far ahead of playback (a measured 20s film existed 17s after Play, 1ms before the take that keyed on it was torn down), so the server records `film_generated` and ends nothing. `reason` is a closed enum; an unrecognised one is `400 invalid_command` and never collapses into a reasonless stop.
