...
| 3 | SUPABASE_SETUP.md migration list is missing 4 of 14 migrations | Doc behind code | `docs/SUPABASE_SETUP.md:9-15` lists 10 migrations ending at `20260919213000_persist_admission_throttle.sql` | `supabase/migrations/` has 14 files; missing are `20260919220000_catalogue_titles.sql`, `20260919221000_search_catalogue_titles.sql` (Discover/TMDB catalogue), `20260919230000_jam_playback_clock.sql` (shared playback clock) and `20260919231000_constrained_catalogue_shortlist.sql` (constraint-aware shortlist, replaces `search_catalogue_titles` with a 10-arg form) — following the doc verbatim yields a project with no catalogue, no shortlist RPC and no playback clock. Gap is widening each time a Discover/playback migration lands without a SUPABASE_SETUP.md update |
...
| 9 | GALTEA_AGENT_SPEC.md's "Discover Agent" / "Story Director" have no code | Doc ahead of code | `docs/GALTEA_AGENT_SPEC.md:14-31` describes an NLU conversational Discover agent that "understand[s] natural-language requests" and asks clarifying questions, plus a Story Director managing votes/conflicts | Still no chat UI or free-text NLU anywhere. The 2026-09-19 "Discover refines" work (`src/discover/RefinementBar.tsx`, `src/catalogue/refinements.ts`) added a *structured* preference rail — fixed chips grounded in literal substrings of canned sentences, not open natural-language understanding or clarifying questions — so it's adjacent to but does not close this gap; `docs/PROJECT_STATE.md` explicitly notes "no mood, tone or pace chip" because there's no NLU. No "Story Director" module anywhere. `docs/PROJECT_STATE.md` correctly frames the spec as not-yet-evaluated, but the spec file itself doesn't restate that framing |
