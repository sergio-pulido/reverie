...
- A typed fal.ai queue adapter exists behind `REVERIE_LIVE_ENABLED` + `FAL_KEY` with a server-owned model allowlist defaulting to `minimax/h3-max/text-to-video`. Portions are whole seconds in `[5, 15]` and the default jam is 20 seconds of 5-second portions, because that is the band the model accepts (RV-16). It is NOT probed: no fal model has been verified, no credentials exist in the repo, and playback start returns a typed `generation_disabled` error until live configuration is provided. A dated probe receipt in `docs/DECISIONS.md` must precede any claim that generation works.
- A live-director path exists for `minimax/h3-max/director`, a realtime WebRTC model: the server brokers the SDP handshake and owns a spend ledger, the browser is the peer, and nothing it generates is stored (`apps/server/director.ts`, `src/lib/directorSession.ts`, `src/screens/JamDirector.tsx`). It is gated behind `REVERIE_DIRECTOR_ENABLED` and is NOT probed — no session has been opened and no stream has been watched.
