...
> conversation at `/search`, and `/discover` now leads there. Sections A and D still apply, and so
...
> kept only until this runbook is rewritten for `/search`. What `/search` does and how it was
