...
- **An outline to steer from.** Every portion carries a one-phrase beat, written with the script. The room reads the beats instead of the screenplay, rewrites one or rejects it, and the story after it is re-derived in one model call and committed as a new revision. Edits queue one at a time per jam, respect the live director's lock window, and the changed beat is sent to any open stream as direction. Verified offline and against the local server with providers off; no cascade has been run against a live model from this repository, so the quality of a rewritten tail is unobserved. The spec is `docs/specs/story-outline.md`.
