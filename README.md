...
- **An outline to steer from.** Every portion carries a one-phrase beat, written with the script. The room reads the beats instead of the screenplay, rewrites one or rejects it, and the story after it is re-derived in one model call and committed as a new revision. Edits queue one at a time per jam, respect the live director's lock window, and the changed beat is sent to any open stream as direction. This is written on the RV-22 branch and has not been tested or run yet, offline or live; the spec is `docs/specs/story-outline.md`.
