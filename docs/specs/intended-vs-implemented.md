...
| Cross-instance serialization of script mutations | Decided and implemented (RV-22) | `withJamLock` and the per-jam outline queue in `apps/server/outline.ts`; `JamStore.commitScript` guards on the boundary and the base revision | The jams router runs only in the container process the director already requires (`docs/DECISIONS.md` 2026-09-20), so the per-jam critical section is real there. Deploying these routers as Vercel functions is not a supported deployment for edits |
| Story outline projection (beats over the script) | Implemented (RV-22) | `src/core/outline.ts` (`buildOutline`, `beatAt`); `summary` on the portion schema; served by `GET /api/jams/:id/outline` (`apps/server/outline.ts`); rendered by `src/screens/OutlinePanel.tsx` on the script screen and in the Studio | Beats are created with the script (`apps/server/outlineWriter.ts`); a revision created before RV-22 shows its beats as missing |
| Outline cascade (re-derive the tail from an edited beat) | Implemented, probed live 2026-09-20 (RV-22) | `src/core/outlineCascade.ts` (prompt for `set` and `reroll`, reply schema, `applyCascade`); `runCascade` in `apps/server/outlineWriter.ts` calls Nebius with one retry | Probed against Nebius on the local stack: a `set` and a `reroll` each re-derived the tail coherently, durations unchanged (receipt in `docs/PROJECT_STATE.md`). Tests still inject the completion |
| Outline edit queue (serialized, envelope-guarded) | Implemented (local only) (RV-22) | `OutlineEditQueue` and the routes in `apps/server/outline.ts`; `outlineEditCommandSchema` in `src/core/outlineEdit.ts` (`requestId`, `expectedRevision`, `mechanism`, `authorId`); `JamStore.commitScript` | In-memory ledger, lost on restart with the script; no authorization on the routes; no lifecycle state refuses an edit any more — a stopped room is between takes and takes edits (RV-23), and `portion_locked` is what protects a beat already with the provider |
| Ways to modify the outline (direct, vote, poll, chat) | Direct implemented (RV-22); vote, poll, chat not implemented | The panel's "Rewrite" (`set`) and "Not this" (`reroll`) in `src/screens/OutlinePanel.tsx`, `mechanism: "direct"` | No tally, poll or chat adapter; `jam_proposals` remains append-only with no update policy |
| Beat delivered as live director direction | Implemented for the imminent beat only, unprobed (RV-22) | After a commit the queue calls `stream.direct({ body, beatIndex, authorId })` on every stream `DirectorStreamRegistry.streamsFor(jamId)` returns; refusals land on the edit record | Never exercised against a live stream; a fake stream in tests records the body, beat index and author. **Open gap, owned by the director contract, not by the outline:** `configure` carries the script once at session open and nothing re-sends it, so a beat edited beyond the imminent one is committed correctly and never reaches an open stream. Closing it needs something pushing a beat as it becomes imminent; no delivery design here does that |
