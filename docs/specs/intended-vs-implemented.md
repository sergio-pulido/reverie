...
| Live director (realtime WebRTC stream, directable while it runs) | Implemented but gated/unprobed | `apps/server/director.ts`, `apps/server/directorSessions.ts`, `apps/server/providers/falDirector.ts`, `src/core/directorProtocol.ts`, `src/lib/directorSession.ts`, `src/screens/JamDirector.tsx` | No session has ever been opened against fal and no stream has been watched. Gated by `REVERIE_DIRECTOR_ENABLED`. Browser is the WebRTC peer, so the server does not see individual prompts; nothing generated is stored |
