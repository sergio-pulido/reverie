...
A take also ends at the end of the film, which on any script shorter than the ceiling is what it reaches first. The provider does not stop at the last beat, so the server ends the session once the take has generated the script's total runtime — `generatedSeconds` reaching it, or the frontier `script_offset_seconds` passing it, whichever comes first (`src/core/directorCompletion.ts`). It is the same teardown a pressed Stop runs: the session settles, the archive closes, the room leaves `playing`, and the take is left in the archive like any other. The trail records `session_complete` before the `session_closed` that follows, which is the only thing that distinguishes a film that ended from a stream that failed. A script whose runtime reads zero never self-completes.
