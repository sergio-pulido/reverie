...
| Outline cascade (re-derive the tail from an edited beat) | Implemented, unprobed (RV-22) | `src/core/outlineCascade.ts` (prompt for `set` and `reroll`, reply schema, `applyCascade`); `runCascade` in `apps/server/outlineWriter.ts` calls Nebius with one retry | No cascade has been run against a live provider from this repository; exercised with injected completions only, so the quality of a rewritten tail is unobserved |
