...
import { SpendAccount } from "./spendLedger";
...
export class FalBudget {
  ...
  /**
   * Wraps an account rather than holding money of its own, so the reserve/settle
   * callers and the commit/refund callers are spending the same total. Given no
   * account it makes one at `budgetUsd`, which is what tests want.
   */
  constructor(private readonly budgetUsd: number, account?: SpendAccount) {
    this.account = account ?? new SpendAccount(budgetUsd);
  }
  ...
  reserve(usd: number): ((actualUsd: number) => void) | null {
    ...
    if (!this.account.commit(usd)) return null;
    this.reservations += 1;
    let settled = false;
    return (actualUsd: number) => {
      ...
      settled = true;
      this.reservations -= 1;
      const billed = Number.isFinite(actualUsd) && actualUsd > 0 ? actualUsd : 0;
      this.account.settle(usd, billed);
    };
  }
}
