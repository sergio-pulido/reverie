...
**Live director boundary.** `minimax/h3-max/director` is a realtime WebRTC model and does not use the queue. `POST /api/jams/:id/director/session` takes the browser's SDP offer, exchanges it for fal's answer server-side, and returns the answer plus a `configure` message built from the jam's script; `POST .../director/session/:sessionId/renew` keeps it alive and `POST .../director/session/:sessionId/end` releases it. The browser is the WebRTC peer, so once the connection is up, direction travels from the client to fal directly and the server does not see individual prompts — it owns session authorization, concurrency and budget only. Sessions bill a 60-second minimum of runtime, so opening one reserves its worst-case cost and an abandoned session is reclaimed without a refund. Gated by `REVERIE_DIRECTOR_ENABLED` on top of `REVERIE_LIVE_ENABLED`. Refusals are typed: `director_disabled`, `budget_exhausted`, `too_many_sessions`, `already_open`, `director_unavailable`. Nothing a director session generates is stored.
