...
**fal.ai boundary.** Calls go through a typed adapter in `apps/server/providers/fal.ts` behind `REVERIE_LIVE_ENABLED` and `FAL_KEY`, with a server-owned model allowlist (`FAL_MODEL` may only select from it), bounded generation concurrency, and per-jam clip-count and spend caps. The default and fallback model is `minimax/h3-max/text-to-video`; the allowlist lives in `apps/server/providers/falModels.ts` as typed specs, because each model carries its own duration band and request body. A queued request is addressed by its application (`minimax/h3-max/requests/<id>`), not by its full slug. Portion durations are whole seconds in `[5, 15]` — the model's own band, enforced at the script boundary. The adapter is not claimed live until a dated probe receipt is recorded. Clients never receive fal URLs or request bodies.

**Live director boundary.** `minimax/h3-max/director` is a realtime WebRTC model and does not use the queue. **This server is the WebRTC peer**, so nothing reaches the provider that did not pass through it and nothing the provider produces leaves unrecorded. `POST /api/jams/:id/director/session` opens the peer connection, sends `configure` built from the jam's script, and begins recording the media track; `POST .../director/session/:sessionId/direct` is the only way to send direction, and records it with its prompt version, author and originating proposal; `GET .../director/session/:sessionId` returns state plus the audit trail; `POST .../renew` keeps it alive, `POST .../end` closes it, stores the recording and releases the reservation; `GET /api/jams/:id/director/recordings/:sessionId` serves the stored recording. Clients never receive fal URLs, request bodies or a route to the model. Sessions bill a 60-second minimum of runtime, so opening reserves the worst-case cost, an abandoned session is reclaimed without a refund, and a handshake the provider refused is released in full. Gated by `REVERIE_DIRECTOR_ENABLED` on top of `REVERIE_LIVE_ENABLED`. Refusals are typed: `director_disabled`, `budget_exhausted`, `too_many_sessions`, `already_open`, `director_unavailable`, `stream_not_ready`. Because the peer connection is long-lived, this path belongs to the container process and cannot run as a serverless function.
