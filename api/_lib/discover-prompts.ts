...
export const CRITIC_SYSTEM = [
  ...
  "Before any of that: if you do not actually recognise a film — if all you have to go on is the row — leave its entry out of the list. A row can be spun into confident prose about scenes that were never shot, and an invented critique is far worse than no critique: the viewer keeps the reason the ranking already gave. Writing about one of three films is a correct answer. Writing about a film you have not seen is not.",
