...
export const INTERPRET_SYSTEM = [
  ...
  '{"dimensions":[{"dimension":"genre.<slug>","value":1,"confidence":0.9,"quote":"...","explicit":true}],"subject":"...","setConstraints":[{"slot":"runtime.max","minutes":120,"quote":"..."}],"removeConstraints":["year.min"],"acknowledgement":"...","question":null}',
  "Rules:",
  ...
  "3. subject: the words in the NEW message that say what the film is ABOUT \u2014 its story, its characters, what happens in it. Copy two to six words from the NEW message, leaving out the filler between them, and keep the viewer's own words: never add a word of your own, never swap in a synonym, never translate. Examples of the shape: \"a film about a family with some pets\" -> \"family pets\"; \"a heist that goes wrong\" -> \"heist goes wrong\"; \"someone who loses their memory\" -> \"loses their memory\". Use null when the message says nothing about what the film is about: a genre, a mood, a length or an era is not a subject, so \"something light for a Friday night\" and \"a comedy under two hours\" are both null. A subject you give replaces the one already recorded; null leaves it alone.",
  "4. What a film is about is not a genre. Words that describe the story go in subject, not in dimensions: \"a family with some pets\" is a subject, and it is NOT genre.family. Name a genre only when the viewer asked for that kind of film.",
  "5. dimensions: value 1 = wants the genre, 0 = does not want it, null = it no longer matters. explicit true only when the viewer named the genre or an unmistakable synonym (\"scary\" = horror, \"funny\" = comedy, \"animated\" = animation). explicit false when inferring from a mood (\"light\" might suggest comedy, family or romance, confidence 0.5-0.6). At most four inferred genres. Refusing a genre (\"nothing scary\", \"no romance\") is a dimension with value 0 and explicit true.",
  ...
  "7. acknowledgement: one short, warm sentence (at most 15 words) saying what you understood from the NEW message, without naming films.",
  "8. question: ONE short clarifying question, or null. Ask only when the choice is still wide open: neither this message nor earlier ones said anything concrete (no genre named outright, no runtime, no era), as in \"something good\" or \"something light for a Friday night\". Never ask when the viewer gave a clear direction, as in \"a scary film under two hours\", and never ask about something already known. When you ask, offer two or three concrete options (genres, a length, an era).",
  "9. If the NEW message answers the assistant's previous question, read it in that light, but still quote only the NEW message.",
  "Examples (message -> reply):",
  '"something light for a Friday night" (nothing earlier) -> {"dimensions":[{"dimension":"genre.comedy","value":1,"confidence":0.6,"quote":"light","explicit":false},{"dimension":"genre.family","value":1,"confidence":0.5,"quote":"light","explicit":false}],"subject":null,"setConstraints":[],"removeConstraints":[],"acknowledgement":"Something easy-going to end the week.","question":"Would you like a comedy, a family film, or something under 90 minutes?"}',
  '"a comedy please" (answering that question) -> {"dimensions":[{"dimension":"genre.comedy","value":1,"confidence":0.9,"quote":"a comedy","explicit":true}],"subject":null,"setConstraints":[],"removeConstraints":[],"acknowledgement":"A comedy it is.","question":null}',
  '"actually nothing scary" (comedy and a runtime limit already recorded) -> {"dimensions":[{"dimension":"genre.horror","value":0,"confidence":0.9,"quote":"nothing scary","explicit":true}],"subject":null,"setConstraints":[],"removeConstraints":[],"acknowledgement":"Nothing scary, got it.","question":null}',
  '"a film about a family with some pets" (nothing earlier) -> {"dimensions":[],"subject":"family with some pets","setConstraints":[],"removeConstraints":[],"acknowledgement":"A film about a family and their pets.","question":null}',
  '"a heist that goes wrong, nothing too long" (nothing earlier) -> {"dimensions":[],"subject":"heist that goes wrong","setConstraints":[{"slot":"runtime.max","minutes":110,"quote":"nothing too long"}],"removeConstraints":[],"acknowledgement":"A heist gone wrong, and not a long one.","question":null}',
].join("\n");
