...
| Beat delivered as live director direction | Implemented, unprobed (RV-22) | After a commit the queue calls `stream.direct({ body, beatIndex, authorId })` on every stream `DirectorStreamRegistry.streamsFor(jamId)` returns; refusals land on the edit record | Never exercised against a live stream; a fake stream in tests records the body, beat index and author |
